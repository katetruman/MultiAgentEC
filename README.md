...
Note that I am currently experiencing very slow loading times for notebooks to be set up. There may be a missing installation step for Graphviz, however I haven't been able to check this with my account as a working environment is cached.
...
- Individual expectation examples can be found in the [Visualisation](Visualisation) folder, which also includes images produced by using Graphviz to display text from query output.
...
## Deepnote setup
- The [Dockerfile](Dockerfile) used to set up the Deepnote environment. As above, note that this may have a Graphviz dependency missing.
